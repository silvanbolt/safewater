...
def run_top3_with_failures(model_path, data_root):
    ...
    for class_name in all_categories:
        ...
        for res in results:
            ...
            if true_idx in (t1, t2, t3):
                ...
                # top-3 failure: record image name and predictions
                top3_names = [model.names[i] for i in (t1, t2, t3)]
    ...
    print("\nConfusion matrix saved as: top3_results_matrix.csv")
    ...
    print("Heatmap saved as: final_top3_heatmap.png")
    plt.show()

    # --- Failure txt ---
    failure_count = len(failures)
    print(f"\nTop-3 failures: {failure_count} / {total_count} ({failure_count / total_count:.2%})")

    with open("top3_failures.txt", 'w') as f:
    ...
    print("Failure list saved as: top3_failures.txt")
...
if __name__ == "__main__":
    ...
    run_top3_with_failures(MODEL_PATH, DATA_ROOT)
